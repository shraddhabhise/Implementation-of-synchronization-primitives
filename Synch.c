...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Init ()
	waitingThreads = new List [Thread]   -- Initializes a waiting threads list  
      endMethod
      ...
      method Lock ()
	var
        	oldIntStat: int                                -- initialize a variable to store state of interrupts 
         	oldIntStat = SetInterruptsTo (DISABLED)	       -- Disable interrupt and store state of interrupt 
		if (IsAcquired==currentThread)		       -- Check if lock is held by a current thread
			FatalError ("Lock is already held..")  -- Display error
		endIf
		--oldIntStat = SetInterruptsTo (DISABLED)
		if !IsAcquired				       
         	        IsAcquired=currentThread	       -- If lock is not acquired give it to current thread
 		else
               		waitingThreads.AddToEnd (currentThread) -- If lock is already held by some other thread, add current thread to waiting list 
               		currentThread.Sleep ()			-- Blocks current thread
		endIf
      	oldIntStat = SetInterruptsTo (oldIntStat)		-- Restore state of interrupts, to which it was before disabling 
        endMethod
	...
	var
        	oldIntStat: int					-- initialize a variable to store state of interrupts
        	t: ptr to Thread				-- Declare t as  pointer to thread
		oldIntStat = SetInterruptsTo (DISABLED)		-- Disable interrupt and store state of interrupt
		if(IsAcquired!=currentThread)			-- Check if lock is held by a current thread
                	FatalError ("Problem")			-- Display error
		endIf
         	-- oldIntStat = SetInterruptsTo (DISABLED)
        	t = waitingThreads.Remove ()	   		-- Remove a thread from head of waiting list  
        	if t 						-- Check if t is not empty
           	   	t.status = READY			-- change status of t from blocked to ready
           	    	readyList.AddToEnd (t)			-- add t to end of a ready list
       	              	IsAcquired=t				-- Give lock to t
		else
			IsAcquired=null				-- if waiting thread list is empty, assign IsAcquired to null 
		endIf
       oldIntStat = SetInterruptsTo (oldIntStat)		-- Restore state of interrupts, to which it was before disabling
       endMethod
      ...
      method IsHeldByCurrentThread () returns bool
		return IsAcquired==currentThread	        -- returns true if lock is held by current thread
        endMethod
