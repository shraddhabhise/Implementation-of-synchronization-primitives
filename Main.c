...
-----------------------------  Test Mutex  ---------------------------------
  ...
  var
    myLock: Mutex = new Mutex      							-- Creates an object of Mutex
    sharedInt: int = 0             							-- Declares a shared variable 		
    thArr: array [7] of Thread = new array of Thread {7 of new Thread }		 	-- Declares an array of 7 threads

  function TestMutex () 			
      myLock.Init ()									-- Calls Init method of mutex
      ...
      ThreadFinish ()									-- Cleans up a thread object
    endFunction
  ...
  function LockTester (waitTime: int)
    ...
    --        actions done within the critical section    --     Print the thread's name and the new value
    ...
    --        actions done outside of the critical section
      ...
      for i = 1 to 10
        ...
        -- Enter
        myLock.Lock()									-- Acquires lock of mutex
        ...
        -- Leave
        myLock.Unlock()									-- Releases lock of mutex
...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    buffer: array [BUFFER_SIZE] of char = new array of char {BUFFER_SIZE of '?'}         --Initializes an array for buffer
    bufferSize: int = 0		
    bufferNextIn: int = 0								-- Global variable
    bufferNextOut: int = 0
    thArray: array [8] of Thread = new array of Thread { 8 of new Thread }
    printlock: Mutex =new Mutex 							-- Creates an object of  mutex lock for printing
    BufferNextOutLock: Mutex = new Mutex						-- Creates an object of mutex lock for shared variable BufferNextOut  
    BufferNextInLock: Mutex = new Mutex							-- Creates an object of mutex  lock for shared variable BufferNextIn 
    Semaphore_fullbuffs: Semaphore= new Semaphore    					-- Creates an object of semaphore for fullbuffs
    Semaphore_emptybuffs: Semaphore= new Semaphore 					-- Creates an object of semaphore for emptybuffs
    empty_buffs: int=5									-- declares a variable to indicate the size of full buffer
    full_buffs: int= 0    								-- declares a variable to indicate the size of empty buffer

  function ProducerConsumer ()
   
    BufferNextOutLock.Init()								-- Calls Init method for BufferNextOutLock  
    BufferNextInLock.Init()								-- Calls Init method for BufferNextInLock 
    printlock.Init()									-- Calls Init method for printlock 
    Semaphore_fullbuffs.Init(full_buffs)						-- Calls Init method for semaphore to initiazie it for empty_buffs variable
    Semaphore_emptybuffs.Init(empty_buffs)						-- Calls Init method for semaphore to initialize it for full_buffs variable
  ...
  function Producer (myId: int)
      ...
      for i = 1 to 5
        -- Perform synchroniztion...		
       	Semaphore_emptybuffs.Down()						-- Performs down operation on global variable empty_buffs
        BufferNextInLock.Lock()							-- Acquires lock for BufferNextInLock
	printlock.Lock()							-- Acquires lock for printing, so value doesnt change before printing 
	-- Add c to the buffer
        buffer [bufferNextIn] = c
        bufferNextIn = (bufferNextIn + 1) % BUFFER_SIZE
	bufferSize = bufferSize + 1
	PrintBuffer (c)								-- Print a character produced by producer
	printlock.Unlock()							-- Releases lock for  printing
        -- Perform synchronization...
	Semaphore_fullbuffs.Up()                                                -- Performs up operation on global variable full_buffs
       BufferNextInLock.Unlock()						-- Releases lock for BufferNextInLock 
  ...
  function Consumer (myId: int)
      ...
      while true
        -- Perform synchroniztion...
       Semaphore_fullbuffs.Down()						-- Performs down operation on global variable full_buffs
        -- Remove next character from the buffer
       BufferNextOutLock.Lock()							-- Acquires  lock for BufferNextOutLock 
	printlock.Lock()							-- Acquires lock for printing
	c = buffer [bufferNextOut]			
        bufferNextOut = (bufferNextOut + 1) % BUFFER_SIZE
 	bufferSize = bufferSize - 1
	PrintBuffer (c)								-- Print a character consumed by consumer
	printlock.Unlock()							-- Releases lock for  printing
        -- Print a line showing the state
	Semaphore_emptybuffs.Up()                                               -- Performs up operation on global variable empty_buffs
	BufferNextOutLock.Unlock()						-- Releases lock for BufferNextOutLock
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  class ForkMonitor
    ...
    fields
      ...
      mutexLock: Mutex			   				-- Declares the monitors lock
      SignalEating: array[5] of Condition  				-- Declare Array for storing conditions for 5 philosophers  
      methods
      Init ()
      PickupForks (p: int)		   				-- When philosopher p wants to eat
      Test(p: int)			   				-- It will test if philosopher can eat or no
      PutDownForks (p: int)		   				-- When philosopher p has finished eating
      PrintAllStatus ()			   				-- It will print current status of philosophers
  endClass
  ...
  behavior ForkMonitor
    ...
    method Init ()
      ...
      var i:int
      status= new array of int { 5 of THINKING}             		-- For each philosopher: HUNGRY, EATING, or THINKING
      SignalEating= new array[5] of Condition { 5 of new Condition}
      for i= 0 to 4
	SignalEating[i].Init()						--initially makes status of all philosophers THINKING through Init
      endFor
      mutexLock = new Mutex						-- Creates object for monitors lock
      mutexLock.Init()							--Initializes monitors lock
    endMethod

    method PickupForks (p: int)
      -- This method is called when philosopher 'p' is wants to eat.
	mutexLock.Lock()						-- Acquires monitors lock
	status[p]=HUNGRY						-- makes status of p philosopher HUNGRY
	self.PrintAllStatus()						-- prints status of p
	self.Test(p)							-- checks if p can eat
	--Using Mesa semantic
	while status[p]!=EATING
		SignalEating[p].Wait( & mutexLock)			-- if p is not eating, call wait
	endWhile
	mutexLock.Unlock()						-- releases monitors lock
	...
	mutexLock.Lock()						--Acquires monitors lock
	status[p]=THINKING						-- make status of p thinking
	self.PrintAllStatus()						-- print status of p
	self.Test((p+1) % 5)						-- check if its left neighbour can eat
	self.Test((p-1) % 5)						-- check if its right neighbour can eat
	mutexLock.Unlock()						-- Releases monitors lock 
    ...
    method Test(p:int)
    --Using mesa semantic
      while status[p]==HUNGRY && status[(p+1) % 5] != EATING && status[(p-1) % 5] != EATING    -- check if p is hungry and its left neighbour is not eating and its right neighbour is not eating
		status[p]=EATING							       -- make status of p EATING
		self.PrintAllStatus()							       -- Print status
		SignalEating[p].Signal( & mutexLock ) 					       -- signal waiting thread 
	endWhile
    endMethod
